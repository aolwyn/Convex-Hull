...
def buildHull(points):
    """ [list of points] -> [List of points on hull]
        While not needed to return list of points on hull, the way I coded the algorithm it is a by product
    """
    # Handle base cases of two or three points
    #
    # [YOUR CODE HERE]
    
    if len(points)==2:    #If only 2 points, simply connect to eachother 
    ...
    elif len(points)==3:  #Needed seperate base case for 3 points as there are multiple orientations that need 3 points to examine
      ...
      else:                                         #If the 3 points do not make a right turn, connected cw
        for i in range(len(points)):
          points[i].highlight = True
          if points[i] == points[-1]:            
    ...
    return(hull_merge(buildHull(points[:mid]),buildHull(points[mid:])))   #Recursive call, merges the hull with the left and right sides of the points
...
def main():
    ...
    sys.argv.append("points1.txt") #Used for running not from command line
    #sys.argv.append("b4.txt")
